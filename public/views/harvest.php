...
<h1 class="text-2xl sm:text-3xl font-bold mb-6 text-green-800">Harvest Management</h1>
    ...
    <?php if (isset($records) && !empty($records)): ?>
        <ul class="space-y-2.5">
            <?php foreach ($records as $record): ?>
                <li class="bg-white/50 border border-white/60 rounded-xl px-3.5 sm:px-4 py-3 text-sm text-slate-700 shadow-sm">Crop ID: <?php echo htmlspecialchars($record['crop_id']); ?> - Quantity: <?php echo htmlspecialchars($record['quantity']); ?>, Quality: <?php echo htmlspecialchars($record['quality']); ?></li>
            <?php endforeach; ?>
        </ul>
    <?php else: ?>
